...
# Claude Code PostToolUse hook — appends a JSONL event line to .claude/memory/events.jsonl
# Invoked by Claude Code on every Write|Edit|NotebookEdit|Bash tool call.
# Hook input arrives on stdin as JSON: { tool_name, tool_input, ... }
# Agent identification comes from $env:CLAUDE_AGENT (set by command files before invoking sub-agents).
