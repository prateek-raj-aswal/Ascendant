...
`auditor` writes `.claude/memory/stories/{story_id}/review.md` with five sections — Summary, Critical Issues, Warnings, Suggestions, Verdict (PASS | FAIL).
...
If you want to review a Design artifact (system-design.yaml), invoke this with `type: Design` and omit story_id; `auditor` will write to `.claude/memory/architecture/design_review.md` instead.

## Success Criteria
- [ ] `review.md` (or `design_review.md`) written
