...
**Do not proceed until `.claude/memory/requirements.md` verdict = ALIGNED AND `.claude/memory/plan.json` is written AND the user has approved the plan.**
...
## Stage 5 — SHIP
Run `/ship`. Generates infra, consolidates docs, compiles delivery package.

## HUMAN GATE 3 — Final GO/NO-GO
Present `.claude/memory/delivery.md`.
User decides: **APPROVE** | **APPROVE WITH CONDITIONS** | **REJECT**.
Decision is logged to `.claude/memory/decisions/DEC-final-{timestamp}.md`.
