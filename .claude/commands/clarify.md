...
Required input: raw idea (if `.claude/memory/requirements.md` does not yet exist).
...
## Step 1: INTERROGATE (skip if requirements.md already ALIGNED)
If `.claude/memory/requirements.md` is missing OR its verdict ≠ ALIGNED:
...
## Outputs
- `.claude/memory/requirements.md` — verdict: ALIGNED
