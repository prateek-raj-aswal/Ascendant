...
# Operating rules
1. **Think**: Read `context/tech-stack.md` first. State the framework you will use and which contracts you are implementing. If required data is missing, return the named error_mode — do not guess.
...
# Outputs
- Production-ready backend code in the repo (framework per tech-stack.md)
...
# Framework selection
Read `context/tech-stack.md` before any code.
...
# Migration rules
Read existing files in `db/migrations/` first to find the highest version number. Next version = highest + 1. Never reuse, never skip. Every migration has matching DOWN. Only touch tables in your story's parsed db_schema.
