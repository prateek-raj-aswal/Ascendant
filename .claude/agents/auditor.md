...
Output: write `.claude/memory/stories/{story_id}/review.md` (Code/Test) or `.claude/memory/architecture/design_review.md` (Design), with all five sections:
...
## DELIVERY mode
Precondition: every story done, SECURITY verdict CLEAR, every story has review.md. Refuse otherwise with `SECURITY_BLOCKED` or `MISSING_REVIEWS`.

Output: write `.claude/memory/delivery.md` with six sections:
...
- DELIVERY: APPROVE only if all AC met, no critical bugs, security CLEAR. Otherwise REJECT or APPROVE WITH CONDITIONS.
